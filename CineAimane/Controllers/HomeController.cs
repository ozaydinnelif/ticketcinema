...
namespace CineAimane.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var nowPlaying = _context.Movies.Where(m => m.StartDate <= DateTime.Now && m.EndDate > DateTime.Now).OrderByDescending(m => m.Id).ToList();
            //var nowPlaying = _context.Movies.OrderByDescending(m => m.Id).ToList();
            var comingSoon = _context.Movies.Where(m => m.StartDate > DateTime.Now).ToList();
